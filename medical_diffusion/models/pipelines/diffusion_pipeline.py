...
class DiffusionPipeline(BasicModel):
    def __init__(self, 
        ...
        self.masked_condition = masked_condition
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int, optimizer_idx:int):
        ...
        # print("0- condition:",condition.shape,condition.max(),condition.min())
        # print("x_0:",x_0.shape,x_0.max(),x_0.min())
        # origin_x_0 = x_0
        if self.masked_condition:
            masked_x_0 = x_0 * condition
        # print("0- masked_x_0:",masked_x_0.shape)
        ...
        if self.latent_embedder is not None:
            ...
            with torch.no_grad():
                ...
                if self.masked_condition:
                    masked_x_0 = self.latent_embedder.encode(masked_x_0)
       ...
       # concat
        # print("masked_x_0:",masked_x_0.shape) # [4, 4, 16, 16, 16]
        # print("condition:",condition.shape) # [4, 1, 128, 128, 128]
        if self.masked_condition:
            # downscale condition to 16x16x16
            condition = F.interpolate(condition, (16,16,16))
            # print("resize - condition:",condition.shape)
            condition = torch.cat([masked_x_0, condition], dim=1)
        # print("origin_x_0:",origin_x_0.shape)
        # condition = torch.cat([origin_x_0, condition], dim=1)
